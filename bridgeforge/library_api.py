...
def inventory_library_api(jar: Path) -> dict:
    ...
    with jar.open("rb") as source:
        for chunk in iter(lambda: source.read(1024 * 1024), b""):
            digest.update(chunk)
    return {"schema_version": 1, "mode": "READ_ONLY_LIBRARY_CLASS_INVENTORY", "jar": jar.name, "sha256": digest.hexdigest(), "class_count": len(classes), "classes": classes}
...
def match_library_imports(mod_directory: Path, inventory: dict, target: TargetProfile) -> dict:
    ...
    return {"schema_version": 1, "mode": "READ_ONLY_LIBRARY_API_MATCH", "mod": mod_directory.expanduser().resolve().name, "inventory_sha256": inventory.get("sha256"), "inventory_namespace": namespace or None, "inventory_packages": packages, "matched_import_count": len(imports) - len(missing), "unmatched_imports": missing, "migration_candidates": candidates}
