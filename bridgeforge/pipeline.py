...
from .workspace import checkpoint, workspace_paths
...
def run_pipeline(workspace: Path, target: TargetProfile, rules: list[Path] | None = None, approved: set[str] | None = None, apply_safe: bool = False, jdk: Path | None = None, api_jars: list[Path] | None = None, dependency_jars: list[Path] | None = None, compile_requested: bool = False, bytecode_file: str | None = None, bytecode_rules: Path | None = None, bytecode_approved: set[str] | None = None, library_registry: dict[str, LibraryRegistryEntry] | None = None, starsector_install: Path | None = None) -> dict:
    ...
    compile_status = None if compile_result is None else compile_result.get("status", "PASS" if compile_result["success"] else "FAILED")
    result = {"schema_version": 1, "target": asdict(target), "scan": {"report": str(scan_report), "manifest": str(scan_manifest), "finding_count": len(scan.findings)}, "plan": {"migration_count": len(plan["migrations"])}, "apply": {"applied_count": len(applied["applied"])}, "bytecode": bytecode, "build_profile": build is not None, "compile_validation": None if build is None else build["compile_validation"], "compile": None if compile_result is None else compile_result["success"], "compile_status": compile_status, "compile_feedback": None if feedback is None else len(feedback["findings"]), "validation": validation, "save_risk": save_risk, "review_bundle": str(bundle)}
